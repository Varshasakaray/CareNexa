...
import { sendBookingNotificationToHelper, sendBookingOTPToPatient } from '../emailVerify/helperEmails.js';
...
export const browseHelpers = async (req, res) => {
    try {
        ...
        }).select('-password -aadhaarNumber -governmentIdProof -emergencyContact -token');
...
export const getHelperDetails = async (req, res) => {
    try {
        ...
        if (!helper || !helper.isActive || helper.verificationStatus !== 'approved') {
...
export const calculatePrice = async (req, res) => {
    try {
        ...
        if (minutesDiff < 15 && minutesDiff > 0) {
            return res.status(400).json({
                success: false,
                message: 'Booking must be done at least 15 minutes prior to appointment time'
            });
        }

        const priceDetails = await calculateBookingPrice(appointmentDate, isEmergency || false);
...
export const createBooking = async (req, res) => {
    try {
        ...
        const { helperId, patientAddress, hospitalName, appointmentTime, isEmergency } = req.body;

        if (!helperId || !patientAddress || !hospitalName || !appointmentTime) {
            return res.status(400).json({
                success: false,
        ...
        await sendBookingNotificationToHelper(
            helper.email,
            helper.fullName,
            {
                patientAddress,
...
export const completeDuty = async (req, res) => {
    try {
        ...
        return res.status(200).json({
            success: true,
            message: 'Duty completed successfully. Patient can now rate your service.',
            data: booking
        });
    } catch (error) {
        return res.status(500).json({
...
export const rateHelper = async (req, res) => {
    try {
        ...
        helper.averageRating = totalRatings > 0 ? (sumRatings / totalRatings).toFixed(2) : 0;
