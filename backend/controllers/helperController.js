...
import { sendHelperApprovalEmail, sendHelperRejectionEmail, sendBookingNotificationToHelper, sendDutyCompletionReminder, sendHelperLoginOTP as sendHelperLoginOTPEmail } from '../emailVerify/helperEmails.js';
...
export const registerHelper = async (req, res) => {
    try {
        ...
        // Validation
        if (!fullName || !age || !mobileNumber || !email || !aadhaarNumber || 
            !address || !pincode || !emergencyContact || !password) {
        ...
        return res.status(201).json({
            success: true,
            message: 'Helper registered successfully. Please complete payment of ₹' + pricing.registrationFee + ' to proceed.',
            data: {
...
export const verifyHelperLoginOTP = async (req, res) => {
    try {
        ...
        // Generate tokens
        const accessToken = jwt.sign({ id: helper._id }, process.env.SECRET_KEY, { expiresIn: '10d' });
        const refreshToken = jwt.sign({ id: helper._id }, process.env.SECRET_KEY, { expiresIn: '30d' });
...
export const getHelperProfile = async (req, res) => {
    try {
        ...
        const helper = await Helper.findById(helperId).select('-password -aadhaarNumber');
        if (!helper) {
            return res.status(404).json({
                success: false,
                message: 'Helper not found'
