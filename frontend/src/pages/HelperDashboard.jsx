...
const HelperDashboard = () => {
  ...
  const activeBookings = bookings.filter((b) =>
    ["accepted", "otp_sent", "otp_verified"].includes(b.status),
  );
              ...
              <div>
                ...
                {helper.helperRegistrationNumber && (
                  <p className="text-sm text-gray-500">
                    ID: {helper.helperRegistrationNumber}
                  </p>
                )}
                <div className="mt-4 flex flex-wrap gap-2">
                  <Badge>{helper.experienceLevel}</Badge>
                  {helper.trustBadge && <Badge variant="default">Trusted</Badge>}
                  <Badge variant="outline">Rating: {helper.averageRating}/5</Badge>
                  <Badge variant="secondary">Total: {helper.totalBookings}</Badge>
                  <Badge className="bg-green-100 text-green-800 hover:bg-green-200 border-none">Done: {helper.completedDuties}</Badge>
                  <Badge className="bg-red-100 text-red-800 hover:bg-red-200 border-none">Cancelled: {helper.cancelledBookings || 0}</Badge>
                </div>
              </div>
                    ...
                    {booking.status === "otp_sent" && (
                      <div className="mt-4">
                        <p className="text-sm font-semibold mb-2">
                          Verify OTP (received from patient):
                        </p>
                        <div className="flex gap-2">
                          <Input
                            ...
                            onChange={(e) =>
                              setOtp({ ...otp, [booking._id]: e.target.value })
                            }
                            className="max-w-xs"
                          />
                          <Button
